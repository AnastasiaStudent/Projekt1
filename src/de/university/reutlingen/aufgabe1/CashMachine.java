...
public class CashMachine {
	...
	public void insertCashCard(CashCard cashCardX) { // Karteneingabe
		...
		case READY: {
			...
			if (cashCardX.getPin() == pinX) {
				...
				accounts[zaehler]
						.setAccountNumber(cashCardX.getAccountNumber());
		...
		case CARD_INSERTED: {
			...
			case PIN_CORRECT:
				...
				if (accounts[zaehler].bankDeposit > accounts[zaehler].overdraft) {
					accounts[zaehler].bankDeposit = accounts[zaehler].bankDeposit
							- amount;
					System.out.println("Ihr Kontoguthaben ist: "
							+ accounts[zaehler].getBankDeposit());

				} else {
					throw new MeinException(
							"-= Impossible! Not enough money in the account =-");
				}
				{
				}
	...
	public void accountStatement() { // Kontoinformationen
		...
		case CARD_INSERTED:
			switch (statePIN) {
			case PIN_CORRECT:
				System.out.println("Ihr Kontoguthaben ist: und weitere Info"
						+ accounts[zaehler].getBankDeposit());
				// WEITERE INFO
